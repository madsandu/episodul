		...
		if ( $this->is_priced_per_product() ) {
			...
			if ( $price ) {
				...
				foreach ( $this->price_index[ 'price' ][ $min_or_max ] as $component_id => $product_id ) {
					...
					if ( $item_qty ) {
						$composited_product  = $this->get_composited_product( $component_id, $product_id );
						$this->$property    += $composited_product->get_price_including_tax( $min_or_max, $item_qty );
					}
				}
		...
		if ( $this->is_priced_per_product() ) {
			...
			if ( $price ) {
				...
				foreach ( $this->price_index[ 'price' ][ $min_or_max ] as $component_id => $product_id ) {
					...
					if ( $item_qty ) {
						$composited_product = $this->get_composited_product( $component_id, $product_id );
						$this->$property   += $composited_product->get_price_excluding_tax( $min_or_max, $item_qty );
					}
				}
