...
function wc_cp_toggle_element( $container, $content, complete ) {
	...
	if ( $container.hasClass( 'closed' ) ) {
		...
		$container.removeClass( 'open' ).addClass( 'closed animating' );
	}
	...
	function WC_CP_Composite( data ) {
		...
		this.init_router = function() {
			...
			if ( $wc_cp_body.hasClass( 'single-product' ) && $wc_cp_body.hasClass( 'postid-' + composite.composite_id ) ) {
				...
				if ( composite.settings.layout !== 'single' && false === composite.router.has_initial_route ) {
		...
		this.Composite_Data_Model = function( opts ) {
			...
			var Model = Backbone.Model.extend( {
				...
				initialize: function() {
					 ...
					 */
					composite.actions.add_action( 'component_totals_changed', this.calculate_totals, 10, this );
				...
				calculate_totals: function() {
					...
					var model                = this,
						base_price           = Number( this.price_data[ 'base_price' ] ),
						base_regular_price   = Number( this.price_data[ 'base_regular_price' ] ),
						base_price_tax_ratio = Number( this.price_data[ 'base_price_tax' ] ),
						base_price_totals;
					...
					this.price_data[ 'total' ]          = base_price_totals.price;
					this.price_data[ 'regular_total' ]  = base_price_totals.regular_price;

					this.price_data[ 'total_incl_tax' ] = base_price_totals.price_incl_tax;
					this.price_data[ 'total_excl_tax' ] = base_price_totals.price_excl_tax;

					$.each( composite.get_components(), function( index, component ) {

						var component_totals = model.get( 'component_' + component.component_id + '_totals' );
						...
						model.price_data[ 'total' ]          += component_totals.price;
						model.price_data[ 'regular_total' ]  += component_totals.regular_price;

						model.price_data[ 'total_incl_tax' ] += component_totals.price_incl_tax;
						model.price_data[ 'total_excl_tax' ] += component_totals.price_excl_tax;
					} );

					var totals = {
						price:          this.price_data[ 'total' ],
						regular_price:  this.price_data[ 'regular_total' ],
						price_incl_tax: this.price_data[ 'total_incl_tax' ],
						price_excl_tax: this.price_data[ 'total_excl_tax' ]
					};
				 ...
				 */
				calculate_subtotals: function( triggered_by ) {
					...
					$.each( composite.get_components(), function( index, component ) {
						...
						var totals        = {
							...
							qty           = model.price_data[ 'quantities' ][ component.component_id ],
							...
							tax_ratio     = model.price_data[ 'prices_tax' ][ component.component_id ],

							regular_price = qty * ( model.price_data[ 'regular_prices' ][ component.component_id ] + model.price_data[ 'addons_prices' ][ component.component_id ] ),
							price         = qty * ( model.price_data[ 'prices' ][ component.component_id ] + model.price_data[ 'addons_prices' ][ component.component_id ] );
		...
		this.Composite_Price_View = function( opts ) {
			...
			var View = Backbone.View.extend( {
				...
				render_task: function( model ) {
					...
					var composite_totals = model.get( 'totals' );

					composite.console_log( 'debug:views', '\nUpdating composite price view' + ( this.is_in_widget ? ' (widget #' + this.is_in_widget + ')' : '' ) + '...' );

					if ( model.get( 'passes_validation' ) && ( model.price_data[ 'per_product_pricing' ] === 'yes' || model.price_data[ 'total' ] !== model.price_data[ 'base_price' ] ) ) {

						if ( composite_totals.price === 0.0 && model.price_data[ 'show_free_string' ] === 'yes' ) {
							this.$el.html( '<p class="price"><span class="total">' + wc_composite_params.i18n_total + '</span>'+ wc_composite_params.i18n_free +'</p>' );
						} else {
							...
							if ( this.suffix !== '' ) {
								...
								if ( this.suffix_contains_price_incl ) {
									formatted_price_incl = '<span class="amount">' + wc_cp_woocommerce_number_format( wc_cp_number_format( model.price_data[ 'total_incl_tax' ] ) ) + '</span>';
									formatted_suffix     =  formatted_suffix.replace( '{price_including_tax}', formatted_price_incl );
								}

								if ( this.suffix_contains_price_excl ) {
									formatted_price_excl = '<span class="amount">' + wc_cp_woocommerce_number_format( wc_cp_number_format( model.price_data[ 'total_excl_tax' ] ) ) + '</span>';
							...
							if ( composite_totals.regular_price > composite_totals.price ) {
								this.$el.html( '<p class="price"><span class="total">' + wc_composite_params.i18n_total + '</span><del>' + formatted_regular_price + '</del> <ins>' + formatted_price + '</ins>' + formatted_suffix + '</p>' );
							} else {
								this.$el.html( '<p class="price"><span class="total">' + wc_composite_params.i18n_total + '</span>' + formatted_price + formatted_suffix + '</p>' );
							}
						}
						...
						if ( current_step.is_component() && current_step.has_options_style( 'thumbnails' ) ) {
							...
							if ( current_step.get_selected_product() > 0 ) {
								...
								if ( current_step.append_results() ) {
									...
									if ( show_hide_el ) {
										current_step.$el.show();
									}

									if ( current_step.component_selection_view.is_relocated() && ! current_step.$component_pagination.find( '.component_options_load_more' ).is( ':visible' ) ) {
			...
			var View = Backbone.View.extend( {
				...
				render: function( event_type ) {
					...
					if ( composite.settings.layout === 'progressive' ) {
						...
						if ( self.is_component() && self.$component_selections.hasClass( 'blocked_content' ) && false !== this.render_html ) {
		...
		this.Component_Options_View = function( component, opts ) {
			...
			var View = Backbone.View.extend( {
				...
				activate_filter: function( event ) {
					...
					event.data.view.update_filters_ui();

					// Block container.
					self.$component_selections.addClass( 'blocked_content' ).block( wc_cp_block_params );
				...
				reset_filters: function( event ) {
					...
					// Get active filters.
					var component_filter_options = self.$component_filters.find( '.component_filter_option.selected' );
				...
				load_more: function() {
					...
					var page  = parseInt( self.find_pagination_param( 'current_page' ) ),
				 ...
				 *
				 * Note: Under paged layouts, instead of rendering the view of the current component, we render ahead and simply call 'refresh_options_state' on every transition.
				 * @see registered 'active_step_changed' handler in Component_Selection_View.
				 *
				 * In most cases, this should be faster than redrawing with every transition :)
				 */
						...
						if ( initial_selection_id !== current_selection_id ) {
							...
							if ( render_type === 'append' && self.append_results() && self.hide_disabled_products() ) {
								...
								var newly_added_thumbnails = thumbnails_container.find( '.component_option_thumbnail_container.new' ),
									of_which_visible       = newly_added_thumbnails.not( '.hidden' );

								newly_added_thumbnails.removeClass( 'new' );

								view.append_results_nesting_count += of_which_visible.length;
				...
				animate_component_options: function( render_type, load_height ) {
					...
					if ( animate_height ) {
						...
						if ( ! render_type === 'append' ) {
							self.$component_selections.removeClass( 'refresh_component_options blocked_content' );
						...
						self.$component_options.animate( { 'height' : new_height }, { duration: 250, queue: false, always: function() {
							self.$component_options.css( { 'height' : 'auto' } );
							setTimeout( function() {
								self.$component_selections.unblock().removeClass( 'refresh_component_options blocked_content' );
								composite.has_transition_lock = false;
							}, 100 );
						} } );
						...
						setTimeout( function() {
							self.$component_selections.unblock().removeClass( 'blocked_content refresh_component_options' );
							composite.has_transition_lock = false;
						}, 250 );
					}
		...
		this.Component_Selection_View = function( component, opts ) {
			...
			var	View = Backbone.View.extend( {
				...
				option_changed: function( event ) {
					...
					if ( selected_product_id !== '' ) {
						...
						setTimeout( function() {
							...
							event.data.view.model.request_details( selected_product_id );
						}, 120 );

					} else {

						// Handle selection resets within the view, but update the model data.
						event.data.view.model.selected_product = '';
						event.data.view.render_response( false );
					}
				...
				update_content: function( content ) {
					...
					var animation_delay = self.get_selected_product_type() === 'bundle' ? 300 : 250;
				...
				animate_updated_content: function() {
					...
					if ( animate_height ) {
						...
						self.$component_content.animate( { 'height': new_height }, { duration: 200, queue: false, always: function() {
							...
							// Scroll...
							wc_cp_scroll_viewport( self.$component_content, { offset: 50, partial: true, scroll_method: 'middle', duration: 200, queue: false, always_on_complete: true, on_complete: function() {

								// Reset height.
								self.$component_content.css( { 'height' : 'auto' } );

								// Unblock component.
								self.$component_selections.unblock().removeClass( 'blocked_content' );
								composite.has_transition_lock = false;
	...
	wc_cp_classes.WC_CP_Step_Factory = function() {
		...
		function WC_CP_Step( composite, $step, index ) {
			...
			this.autoload_compat_options = function() {
				...
				var is_component = self.is_component(),
					component    = is_component ? self : false;

				if ( is_component && wc_composite_params.compat_options_autoload === 'yes' ) {
					if ( composite.settings.layout !== 'single' && is_component && self.has_options_style( 'thumbnails' ) ) {
						...
						if ( component.append_results() && component.hide_disabled_products() ) {
							...
							var load_more        = component.$component_pagination.find( '.component_options_load_more' ),
								results_per_page = component.find_pagination_param( 'results_per_page' );
			...
			this.unblock_step_inputs = function() {
				...
				this.$el.find( 'select.disabled_input, input.disabled_input' ).removeClass( 'disabled_input' ).prop( 'disabled', false );
			...
			this.block_step_inputs = function() {
				...
				this.$el.find( 'select, input' ).addClass( 'disabled_input' ).prop( 'disabled', 'disabled' );
